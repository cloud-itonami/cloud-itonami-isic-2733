(ns wiringdevmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control molding/assembly/test-line
  equipment directly... does NOT authorize or execute
  maintenance/equipment operations... does NOT self-issue an
  electrical-safety certification mark') implemented faithfully. The
  single invariant under test:

    WiringDeviceAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Wiring Devices Plant
    Operations Governor would reject;
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :switch}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :switch (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-molding-line :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the electrical-safety certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))

(deftest invalid-contact-resistance-milliohm-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:contact-resistance-milliohm 999999999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-contact-resistance-milliohm} (-> (store/ledger db) last :basis)))
    (is (not= 999999999.0 (:contact-resistance-milliohm (store/batch db "batch-001"))) "fabricated contact-resistance-milliohm never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "molder-001" :severity :moderate
                                            :description "contact resistance anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "molder-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
