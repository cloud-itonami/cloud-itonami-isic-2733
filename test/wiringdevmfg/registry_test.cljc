(ns wiringdevmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [wiringdevmfg.registry :as r]))
...
(deftest shipment-that-pushes-past-quantity-exceeds
  (is (true? (r/shipment-quantity-exceeded?
              {:quantity-units 200.0 :shipped-units 180.0} 100.0))))

(deftest shipment-exactly-at-quantity-does-not-exceed
  (is (false? (r/shipment-quantity-exceeded?
               {:quantity-units 200.0 :shipped-units 180.0} 20.0))
...
(deftest missing-quantity-is-not-flagged-exceeded
  ...
  (is (false? (r/shipment-quantity-exceeded? {:quantity-units 200.0} nil))))

;; ----------------------------- product-type-valid? -----------------------------

(deftest known-product-types-are-valid
  (doseq [pt [:switch :socket-outlet :plug :junction-box]]
...
;; ----------------------------- contact-resistance-milliohm-valid? -----------------------------

(deftest typical-contact-resistance-milliohm-is-valid
  (is (r/contact-resistance-milliohm-valid? 0.0))
  (is (r/contact-resistance-milliohm-valid? 5.0))
  (is (r/contact-resistance-milliohm-valid? 1000.0))
  (is (r/contact-resistance-milliohm-valid? 20000.0)))

(deftest negative-contact-resistance-milliohm-is-invalid
  (is (not (r/contact-resistance-milliohm-valid? -1.0))))

(deftest excessive-contact-resistance-milliohm-is-invalid
  (is (not (r/contact-resistance-milliohm-valid? 999999999.0)))
  (is (not (r/contact-resistance-milliohm-valid? 20000.01))))

(deftest non-numeric-or-missing-contact-resistance-milliohm-is-invalid
  (is (not (r/contact-resistance-milliohm-valid? nil)))
  (is (not (r/contact-resistance-milliohm-valid? "1000"))))
...
(deftest history-is-append-only
  (let [c1 (r/register-maintenance "mnt-1" "molder-001" 0)
        hist (r/append [] c1)
        c2 (r/register-maintenance "mnt-2" "molder-001" 1)
