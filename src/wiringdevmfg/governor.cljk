(ns wiringdevmfg.governor
  "Wiring Devices Plant Operations Governor -- the independent
  compliance layer that earns the WiringDeviceAdvisor the right to
  commit. The advisor has no notion of whether a piece of equipment it
  wants to schedule maintenance against has actually been
  ...
  draft-schedule) molding/assembly/test-line equipment, whether a
  proposal secretly tries to self-issue an electrical-safety
  certification mark (an authority this actor never holds), whether a
  ...
  coordination proposal and becomes direct molding/assembly/test-line-
  ...
  *reject* a proposal and fall back to HOLD.
  ...
  `:wiring-devices-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    3. Closed effect allowlist     -- is the PROPOSAL's own `:effect`
                                       ...
                                       hallucinated `:molder/actuate`
                                       or `:test-bench/run`) is the
                                       'direct molding/assembly/test-
    ...
    9. Shipment quantity exceeded  -- for `:coordinate-shipment`,
                                       ...
                                       ground truth from the batch's own
   ...
   13. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [wiringdevmfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  assembly/test-line-equipment-control effect."
...
(defn- equipment-control-blocked-violations
  ...
  Anything else (direct molding/assembly/test-line-equipment control, a
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :equipment-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は成形・組立・試験設備の直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- equipment-actuate-blocked-violations
  ...
  to directly actuate molding/assembly/test-line equipment -- this
  actor may only ever propose/schedule a DRAFT maintenance window,
  never actuate the equipment directly. No override, ever."
  [{:keys [op]} proposal]
  (when (and (= op :schedule-maintenance)
             (true? (:actuate-equipment? (:value proposal))))
    [{:rule :equipment-actuate-blocked
      :detail "成形・組立・試験設備の直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))
...
(defn- certification-authority-blocked-violations
  ...
  to self-issue an electrical-safety certification mark (e.g. UL/CE/
  IEC compliance) -- an authority exclusively reserved to the
  accredited certification body, never this actor. No phase and no
  ...
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-certification? payload))
      [{:rule :certification-authority-blocked
        :detail "電気安全認証(UL/CE/IEC等)の自己発行提案は恒久的に禁止 -- 認証機関の専権事項"}])))
  ...
  (when (= op :coordinate-shipment)
    (let [{:keys [batch-id units]} (:value proposal)
      ...
      (cond
        ...
        (not (registry/shipment-quantity-exceeded-checkable? b units))
        [{:rule :shipment-quantity-exceeded
          :detail "生産量/既存出荷実績/申請量のいずれかが数値として確定できない -- 空き容量を検算できないため出荷しない"}]

        (registry/shipment-quantity-exceeded? b units)
        [{:rule :shipment-quantity-exceeded
...
(defn- invalid-contact-resistance-milliohm-violations
  "For `:log-production-batch`, if the patch declares a
  `:contact-resistance-milliohm` that is not a physically plausible
  micro-ohmmeter reading, reject rather than let fabricated/
  sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [milliohm (:contact-resistance-milliohm (:value proposal))]
      (when (and (some? milliohm) (not (registry/contact-resistance-milliohm-valid? milliohm)))
        [{:rule :invalid-contact-resistance-milliohm
          :detail (str milliohm "mΩ は物理的に妥当な接触抵抗試験値の範囲外")}]))))
...
(defn check
  "Censors a WiringDeviceAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-contact-resistance-milliohm-violations request proposal)
