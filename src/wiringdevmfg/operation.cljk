(ns wiringdevmfg.operation
  "WiringDeviceOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (WiringDeviceAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Wiring Devices Plant Operations Governor (:govern) and the rollout
  ...
  (:require [langgraph.graph :as g]
            ...
            [wiringdevmfg.advisor :as advisor]
...
(defn build
  "Compiles a WiringDeviceOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `wiringdevmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; WiringDeviceAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Wiring Devices Plant Operations Governor -- independent censor.
      (g/add-node :govern
