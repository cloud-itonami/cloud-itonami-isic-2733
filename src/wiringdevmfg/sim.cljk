(ns wiringdevmfg.sim
  ...
  assembly/test-line equipment directly (permanently blocked, no
  override), a proposal that tries to self-issue an electrical-safety
  ...
  an implausible contact-resistance-milliohm reading, and a
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on molder-001 (verified, registered, molding press -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on molder-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on test-bench-002 (UNVERIFIED/unregistered test bench -> HARD hold) ==")
    ...
    (println "== coordinate-shipment ship-3 on batch-002 (100 units would exceed quantity 200 vs shipped 180 -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on molder-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible contact-resistance-milliohm -> HARD hold ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue an electrical-safety certification -> HARD hold, PERMANENT ==")
