(ns wiringdevmfg.phase
  "Phase 0->3 staged rollout for the ISIC 2733 (wiring devices:
  switches, socket-outlets, plugs, junction boxes) plant-operations
  coordination actor.
  ...
  consequence (a molding/assembly/test-line-equipment maintenance
  window means production downtime, and the equipment ends up actually
  touched); it is always a human plant supervisor's call.
  `wiringdevmfg.governor`'s `equipment-actuate-blocked-violations`
...
(defn verdict->disposition
  "Map a Wiring Devices Plant Operations Governor verdict to a base
