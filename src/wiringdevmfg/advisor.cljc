(ns wiringdevmfg.advisor
  "WiringDeviceAdvisor -- the *contained intelligence node* for the
  ISIC 2733 (wiring devices: switches, socket-outlets, plugs, junction
  boxes) plant-operations coordination actor.

  It normalizes production-batch patches (product-type/contact-
  resistance-milliohm/quantity/defect-rate), drafts a molding/
  assembly/test-line-equipment maintenance scheduling proposal against
  a piece of equipment, drafts a safety-concern flag, and drafts an
  outbound wiring-device shipment coordination proposal against a
  ...
  a committed record and NEVER a real molding/assembly/test-line-
  equipment actuation, freight dispatch, or electrical-safety
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `wiringdevmfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `wiringdevmfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; assembly/test-line-equipment-control
                                 ; effect
  ...
  `wiringdevmfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  contact-resistance-milliohm, quantity, defect-rate or verification
  status. High confidence, low stakes -- administrative logging, not
...
(defn- schedule-maintenance
  "Draft a molding/assembly/test-line-equipment maintenance-window
  scheduling proposal against a piece of equipment. The advisor
  reports what it can see (equipment verified?/registered?) in its
  rationale, but `wiringdevmfg.governor` NEVER trusts this report --
  it independently re-derives verified?/registered? from the
  equipment's own stored fields before any commit is possible."
...
(defn- flag-safety-concern
  "Draft a contact-overheating/electrical-safety/dielectric-withstand-
  test-hazard concern. ALWAYS `:stake :coordination/safety-concern` --
  ...
  administrative technicality). See `wiringdevmfg.phase`: no phase
  ever adds this op to a phase's `:auto` set; `wiringdevmfg.governor`
  also always escalates on `:coordination/safety-concern`. Two
...
(defn- coordinate-shipment
  "Draft an outbound wiring-device shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed unit quantity -- it does NOT invent one, and
  `wiringdevmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたは配線器具(スイッチ・コンセント・プラグ・ジャンクション"
       "ボックス)製造工場プラント運用コーディネーターの助言者です。"
       ...
       "成形・組立・試験等の設備の直接操作(actuate)を絶対に提案してはいけません"
       "(この actor は提案のみを行い、実行は一切行いません)。"
       "電気安全認証(UL/CE/IEC等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `wiringdevmfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :wiringdevmfg-advisor-proposal
   :op         (:op request)
