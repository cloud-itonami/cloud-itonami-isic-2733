(ns wiringdevmfg.store
  "SSoT for the ISIC 2733 (wiring devices) plant-operations coordination
  actor, behind a `Store` protocol so the backend is a swap, not a
  rewrite -- the same seam every `cloud-itonami-isic-*` actor in this
  fleet uses.

  Scope note: like its siblings (`cloud-itonami-isic-2790`'s own
  `otherelecmfg.store`, `cloud-itonami-isic-2710`'s own
  `elecequipmfg.store`), this build ships a single `MemStore` backend
  only (atom of EDN) -- the deterministic default for dev/tests/demo,
  no deps. Per docs/adr/0001-architecture.md Decision 1, this vertical
  is self-contained (no external wiring-device-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/contact-resistance-milliohm/
                             quantity/defect-rate record. `:verified?`
                             marks whether the batch's own claims have
                             actually been QC-inspected (never inferred
                             from a routine intake patch);
                             `:registered?` marks whether it is on file
                             in the plant's production ledger;
                             `:shipped-units` tracks the batch's own
                             cumulative-shipped ground truth.
    - `equipment`         -- a molding/assembly/test-line unit's own
    ...
    - `maintenance`       -- a scheduled molding/assembly/test-line
                             maintenance-window DRAFT against a piece
                             of equipment (`wiringdevmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound wiring-device shipment
                             DRAFT (`wiringdevmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (wiringdevmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (wiringdevmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :switch
                :model "SW-1G-16A" :contact-resistance-milliohm 8.0
                :quantity-units 500.0 :defect-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 100.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :socket-outlet
                :model "SKT-2G-13A" :contact-resistance-milliohm 12.0
                :quantity-units 200.0 :defect-rate-percent 0.5
                :verified? true :registered? true
                :shipped-units 180.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :plug
                :model "PLG-13A-FUSED" :contact-resistance-milliohm 6.0
                :quantity-units 3000.0 :defect-rate-percent 1.5
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"molder-001" {:id "molder-001" :kind :molding-press
                  :verified? true :registered? true
                  :last-maintenance-date "2026-05-01"}
   "test-bench-002" {:id "test-bench-002" :kind :contact-test-bench
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `wiringdevmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `wiringdevmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  molding-press unit (schedulable for maintenance), one UNVERIFIED/
  unregistered contact-test-bench unit (blocks any maintenance
